...
scroll anchoring during reflow. `chrome-layout.js` must not override the reader's position.
...
| whether the margin card shows, and which target's threads | the reader's standing target: focus on the target or inside it, its margin cluster, or the card | `margin-projection.js`'s `followStanding` on focus arrival and let-go, `pressAway` for a press outside the card, its target, and its cluster, and the explicit opens (`t`, a marker, a mark) |
