...
- `obligation`, when present, freezes the answer the event owned at capture. Its
  ...
  of that thread. `markup` names a page action and takes a stamped version whose
  markup records it. `receipt` names a request and takes `leaf receipt`. Until
  ...
  pick before the Done its Ask waits for, or a message a newer one in its thread
  answers through.
