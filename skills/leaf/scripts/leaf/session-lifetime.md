...
| Codex delivery record | the host state home's session records | the detached adapter or an embedded App Server host | an unaccepted record is inactive while the session owns no page; an accepted record moves under `history/` after every batch is receipted; a live record goes at the scan that finds its pages all gone, an archived one at the next archiving |
