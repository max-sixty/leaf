...
def test_reading_keys_follow_the_focused_pane_without_moving_its_sibling(
    ...
    ranges = page.evaluate(
        """() => Object.fromEntries(['left-reading', 'right-reading'].map(id => {
          const body = document.querySelector(`#${id} > :not(header, footer)`);
    ...
    page.wait_for_function(
        "() => document.querySelector('#left-reading > :not(header, footer)').scrollTop > 0"
    )
    ...
    page.keyboard.press("d")
    page.wait_for_function(
        "() => document.querySelector('#right-reading > :not(header, footer)').scrollTop > 0"
    )
    page.wait_for_timeout(250)
...
def test_covering_panel_keeps_focus_on_a_nested_reading_region(browser, serve):
    ...
    page.evaluate(
        """async () => {
          ...
          list.append(filler);
          registerReadingRegion({ id: 'nested-reading', host: nested, body: nested });
        }"""
    )
...
def test_a_wheel_reading_without_focus_becomes_the_bounded_pane_subject(browser, serve):
    # A short run between the right pane's opening line and its landmark, so the opening
    # line can sit under the banner while the landmark stands high enough in the window
    # for the pane, once it scrolls on its own, to put it back at the same height.
    source = READING_REGIONS_PAGE.replace(
        '<div style="height: 320px"></div>', '<div style="height: 60px"></div>'
    )
    page = open_page(browser, serve(source))
    resized(page, 520, 900)
    pane_posture(page, page.locator("#left-reading"), "flow")
    ...
    # Leave the pane's opening line inside the window but hidden by the banner. The
    # user is on the landmark below it, not on a line they cannot see.
    start = page.locator("#right-start")
    goal = page.evaluate(
        """() => document.querySelector('.lf-banner').getBoundingClientRect().bottom -
          document.querySelector('#right-start').getBoundingClientRect().height - 10"""
    )
    page.mouse.wheel(0, start.bounding_box()["y"] - goal)
    page.wait_for_function(
        "([el, goal]) => Math.abs(el.getBoundingClientRect().top - goal) < 2",
        arg=[start.element_handle(), goal],
    )
    scroll_settled(page)
    covered = page.evaluate(
        """() => [document.querySelector('#right-start').getBoundingClientRect().toJSON(),
          document.querySelector('.lf-banner').getBoundingClientRect().bottom]"""
    )
    assert 0 < covered[0]["bottom"] < covered[1], covered
    # How far the landmark stands below the top of what its scroller shows: the page's
    # banner covers the top of the window, and scroll-padding says by how much.
    below_band = """([landmark, box]) => landmark.getBoundingClientRect().top -
      (box === document.scrollingElement ? 0 : box.getBoundingClientRect().top) -
      (parseFloat(getComputedStyle(box).scrollPaddingTop) || 0)"""
    relative = page.evaluate(
        below_band,
        [
            right_landmark.element_handle(),
            page.evaluate_handle("() => document.scrollingElement"),
        ],
    )
    ...
    assert right.evaluate("el => el.scrollTop") > 0
    after = page.evaluate(
        below_band, [right_landmark.element_handle(), right.element_handle()]
    )
    assert after == pytest.approx(relative, abs=3)
...
def test_revision_restoration_yields_to_input_while_a_diagram_loads(
    ...
    source = (
        ...
        .replace(
            ...
            '<div><p id="other-start">Another reading with enough words to preserve its landmark.</p>'
            '<div style="height: 1000px"></div></div></lf-pane></lf-tab></lf-tabs></main>',
        )
    )
    ...
    try:
        ...
        if gesture == "wheel":
            ...
            page.wait_for_function(
                "() => document.querySelector('#left-reading > :not(header, footer)').scrollTop > 150"
            )
        elif gesture == "focus":
...
@pytest.mark.parametrize("named", [False, True], ids=["unnamed-bodies", "named-bodies"])
def test_a_new_revision_restores_each_panes_semantic_landmark(browser, serve, named):
    """A pane body is the region's scroller whether or not its author named it.

    A named element's scroll offset otherwise crosses a revision as a raw number, which
    would undo the landmark: the new material above it moves the passage, not the pixel.
    """
    ...
    before = page.evaluate(
        """() => ['left-landmark', 'right-landmark'].map(id => {
          ...
          return landmark.getBoundingClientRect().top -
            landmark.closest('lf-pane > :not(header, footer)').getBoundingClientRect().top;
        })"""
    )
...
def test_review_queue_links_are_its_only_navigator_and_keep_both_readings(
    ...
    # Short, but tall enough to hold the workspace, so each pane scrolls on its own.
    resized(page, 1100, 640)
    pane_posture(page, page.locator("#review-detail"), "bounded")
    queue = page.locator("#review-queue > :not(header, footer)")
    detail = page.locator("#review-detail > :not(header, footer)")
...
def test_thread_travel_reveals_a_review_detail_in_its_pane_only(browser, serve):
    ...
    # Open Threads first: the panel narrows the page, and the panes' reflow is not the
    # travel under test.
...
def test_current_and_proposed_results_share_one_review_and_flow_in_order(
    ...
    resized(page, 1200, 900)
    pane_posture(page, current, "bounded")
    ...
    pane_posture(page, current, "flow")
    boxes = [
        current.evaluate("el => el.getBoundingClientRect().toJSON()"),
        proposed.evaluate("el => el.getBoundingClientRect().toJSON()"),
    ]
...
def test_each_comparison_result_keeps_its_own_comment_destination(browser, serve):
    ...
    scroll_settled(page, "#comparison-current > :not(header, footer)")
    ...
    scroll_settled(page, "#comparison-proposed > :not(header, footer)")
...
def test_a_pane_frame_comment_preview_is_not_confined_to_its_body(browser, serve):
    ...
    geometry = preview.evaluate(
        """card => {
          ...
          const pane = document.querySelector('#left-reading > :not(header, footer)')
            .getBoundingClientRect();
          return {
            box: box.toJSON(),
            pane: pane.toJSON(),
...
""",
    ...
    page.keyboard.press("g")
    chips = page.locator(f'{CHIPS}[data-lf-go-to-kind="Link"]')
    expect(chips).to_have_count(4)
    first_code = chips.first.get_attribute("data-lf-hint-code")
    assert first_code
    ...
    assert prepare_href
    code = chips.nth(1).get_attribute("data-lf-hint-code")
    assert code
...
def test_the_ask_walk_position_shares_the_shortcut_line(browser, serve):
    """Navigation state keeps one quiet place while its destination and face change."""
    page = open_page(browser, serve(ASKS_PAGE))
    ...
    geometry = page.evaluate(
        """() => {
          ...
          const status = box('.lf-bottom-status');
          const line = box('.lf-shortcut-bar');
          const positionStyle = getComputedStyle(
            document.querySelector('.lf-walk-position'));
          const statusStyle = getComputedStyle(document.querySelector('.lf-bottom-status'));
...
def test_a_failed_ask_reveal_does_not_register_an_arrival(browser, serve):
    ...
    try:
        ...
        assert take_browser_errors(page) == [
            "leaf: Presentation failed: ask reveal probe"
        ]
    finally:
...
def test_a_questions_digits_are_drawn_whole(browser, serve):
    ...
    for options, sitting in [
        ...
        page.keyboard.press("Tab")
        for id_ in options:
            chip = page.locator(f"#{id_} > .lf-key-badge")
...
def test_the_thread_walk_stays_inline_until_threads_is_opened(browser, serve):
    ...
    expect(
        page.locator(f'.lf-thread[data-id="{roots[1]}"] > .lf-thread-summary')
    ).to_be_focused()
...
def test_a_walked_thread_leaves_through_what_holds_it(browser, serve):
    ...
    # From a heading the user stood on, the same way out: the landing is the element
...
def test_a_layer_is_left_the_same_way_however_it_was_reached(browser, serve):
    ...
    # where the user came from rather than where they are put back.
    marker.focus()
    page.keyboard.press("Enter")
    expect(preview).to_be_visible()
    expect(threads[0]).to_be_focused()
...
def test_the_pointer_over_a_comment_lights_the_passage_it_is_about(browser, serve):
    ...
    page.locator(".lf-thread").filter(has_text="on the first").first.locator(
...
""",
  ...
  #crowded-neighbor { left: 80px; top: 400px; }
    ...
    under_index = page.locator(CHIPS).evaluate_all(
        """(chips, code) => chips.findIndex(chip => chip.dataset.lfHintCode === code)""",
        under_code,
    )
...
def test_target_mnemonics_filter_the_generated_map_without_renumbering_hints(
    ...
    initial_kinds = set(
        chips.evaluate_all("els => els.map(el => el.dataset.lfGoToKind)")
    )
    assert {"Link", "Fold", "Control"} <= initial_kinds, initial_kinds
    assert chips.evaluate_all(
        "els => els.every(el => el.textContent === el.dataset.lfHintCode)"
    ...
    mixed_codes = mixed.evaluate_all(
        ...
        "[el.dataset.lfGoToMarginEntry, el.dataset.lfHintCode]))"
    )
    ...
    target_route = page.locator(
        '.lf-shortcut-bar .lf-shortcut[data-lf-command-ids~="navigation.target"]'
    )
    assert target_route.locator("kbd").evaluate_all(
        "keys => keys.map(key => key.textContent)"
    ) == ["g", "t", "letters"]
    assert target_route.locator("kbd").evaluate_all(
        "keys => keys.map(key => key.dataset.lfSequenceStepState)"
    ...
    assert (
        set(
            page.locator(CHIPS).evaluate_all(
                "els => els.map(el => el.dataset.lfGoToKind)"
            )
        )
        == initial_kinds
    )
    ...
    assert page.evaluate(
        """() => ({
          context: document.querySelector('.lf-go-to-status').checkVisibility(),
...
def test_a_transient_notice_does_not_move_generated_address_hints(browser, serve):
    ...
    page.keyboard.press("g")
    fixed_link_selector = (
        f'{CHIPS}[data-lf-go-to-kind="Link"][data-lf-go-to-target="lk2"]'
    )
...
def test_the_g_chord_reaches_named_surfaces_and_visible_targets(browser, serve):
    ...
    for command, steps, states, words in [
        ...
        hint = line.locator(
            f'.lf-shortcut:not([hidden])[data-lf-command-ids~="{command}"]'
        )
        expect(hint).to_have_count(1)
        sequence = hint.locator(".lf-binding-sequence")
        assert (
            sequence.evaluate(
        ...
        assert (
            sequence.evaluate(
                "el => [...el.querySelectorAll(':scope > kbd')].map(k => k.dataset.lfSequenceStepState)"
            )
            == states
        )
    ...
    def disclosure_state():
        return page.evaluate(
            """() => ({
              ...
              commands: [...document.querySelectorAll('.lf-shortcut-bar .lf-shortcut:not([hidden])')]
                .flatMap(hint => (hint.dataset.lfCommandIds || '').split(' ').filter(Boolean)),
            })"""
        )
    ...
    page.evaluate("() => document.scrollingElement.scrollTo(0, 0)")
    page.keyboard.press("g")
    expect(page.locator(f'{CHIPS}[data-lf-go-to-kind="Link"]')).to_have_count(2)
    assert page.evaluate(
        """() => {
    ...
    page.keyboard.press("g")
    expect(page.locator(f'{CHIPS}[data-lf-go-to-kind="Link"]')).to_have_count(0)
    page.keyboard.press("Escape")
    ...
    page.keyboard.press("g")
    expect(page.locator(f'{CHIPS}[data-lf-go-to-kind="Fold"]')).to_have_count(0)
    page.keyboard.press("Escape")
    ...
    page.keyboard.press("g")
    controls = page.locator(f'{CHIPS}[data-lf-go-to-kind="Control"]')
...
KEY_LINE_HINTS = """() => [...document.querySelectorAll('.lf-shortcut-bar .lf-shortcut')]
  .filter(chip => !chip.hidden)
  .map(chip => ({
    commands: chip.dataset.lfCommandIds,
...
""",
    ...
    assert old_code == "d"
    expect(page.locator(f'{CHIPS}[data-lf-go-to-target="bottom-link"]')).to_have_count(
        0
    )
    page.keyboard.press("Tab")
    expect(page.locator(f'{CHIPS}[data-lf-go-to-target="top-link-one"]')).to_have_class(
    ...
    expect(page.locator(f'{CHIPS}[data-lf-go-to-target="top-link-one"]')).to_have_count(
        0
    )
...
def test_only_controls_and_boxes_with_something_out_of_sight_take_a_tab_stop(
    ...
    stops = page.evaluate(
        """() => [...document.querySelector('.lf-command-reference').querySelectorAll('*')]
    ...
    for _ in range(6):
        ...
        active = page.evaluate(
            """() => {
              ...
              return {inside: document.querySelector('.lf-command-reference').contains(e),
                      name: e.getAttribute('aria-label') || e.className || e.tagName};
            }"""
        )
        assert active["inside"], f"Tab left the command reference for {active['name']}"
        seen.add(active["name"])
    assert "Back to more shortcuts" in seen, seen

    page.keyboard.press("Shift+Tab")
    assert page.evaluate(
        "() => document.querySelector('.lf-command-reference').contains(document.activeElement)"
    )
    page.keyboard.press("Escape")
...
def test_the_reference_runs_available_commands_and_explains_the_rest(browser, serve):
    ...
    geometry = help_el.evaluate(
        """help => ({
          ...
          keys: [...help.querySelectorAll('tr:not([hidden]) .lf-binding-sequence')]
            .map(key => ({
              top: key.getBoundingClientRect().top,
              right: key.getBoundingClientRect().right,
              actionTop: key.closest('tr').querySelector('.lf-command-reference-action')
    ...
    matches = help_el.locator(
        ...
        """rows => rows.map(row => {
          ...
          const scope = row.querySelector('.lf-command-reference-scope').getBoundingClientRect();
          const key = row.querySelector('.lf-binding-sequence').getBoundingClientRect();
          return {cellLeft: cell.left, scopeLeft: scope.left,
                  keyTop: key.top,
                  actionTop: row.querySelector('.lf-command-reference-action')
                    .getBoundingClientRect().top,
                  keyRight: key.right,
    ...
    assert "command.reference.focus.walk" in tab_matches, tab_matches
    search.fill("w")
    w_results = help_el.locator("tr[data-lf-command]:visible").evaluate_all(
        "rows => rows.map(row => row.dataset.lfCommand)"
    )
    assert w_results.index("draw.mode.exit") < w_results.index("page.down"), w_results
    w_matches = help_el.locator(
        ".lf-command-reference-binding-matches tr[data-lf-command]:visible"
    ).evaluate_all("rows => rows.map(row => row.dataset.lfCommand)")
    assert "draw.mode.exit" in w_matches, w_matches
    ...
    cancel_reaction = help_el.locator(
        '.lf-command-reference-command[data-lf-command="reaction.cancel"]'
    )
    page.keyboard.press("ArrowDown")
    expect(search).to_be_focused()
...
def test_the_reference_reads_the_same_way_twice(browser, serve):
    ...
    for _ in range(2):
        ...
        seen.append(
            page.evaluate(
                "() => [...document.querySelectorAll('.lf-command-reference h3')].map(h => h.textContent)"
            )
        )
        page.close()
...
def test_the_g_chord_selects_a_visible_tab_hint(browser, serve):
    ...
    page.keyboard.press("g")
    expect(page.locator(f'{CHIPS}[data-lf-go-to-kind="Tab"]')).to_have_count(2)
...
def test_generated_hints_branch_after_the_single_letter_alphabet(browser, serve):
    ...
    browse_route = line.locator(
        '.lf-shortcut:not([hidden])[data-lf-command-ids~="navigation.target.next"]'
    )
...
""",
    ...
    expect(page.locator(".lf-go-to-hints")).to_have_attribute("aria-hidden", "true")
    ...
    expect(line.locator('[data-lf-command-ids="navigation.go-to.back"]')).to_have_class(
        re.compile(r"\blf-sequence-command\b")
    )
    ...
    expect(page.locator(f'{CHIPS}[data-lf-go-to-target="first"]')).to_have_class(
        re.compile(r"\blf-current\b")
    )
...
def test_the_arrows_say_which_way_the_section_under_the_user_goes(browser, serve):
    ...
    chips = page.evaluate(
        "() => [...document.querySelectorAll('.lf-shortcut-bar .lf-shortcut')]"
        ".map(c => c.textContent)"
    )
...
def test_reference_accepts_native_popover_dismissal_across_modal_entry(browser, serve):
    ...
    thread = page.locator(".lf-thread").first.locator(":scope > .lf-thread-summary")
    thread.focus()
    page.keyboard.press("g")
    page.keyboard.press("Shift+v")
    versions = page.locator(".lf-version-menu")
...
def test_a_scope_cannot_give_one_live_key_two_meanings(browser, serve):
    ...
    page.keyboard.press("?")
    page.keyboard.press("?")
    expect(page.get_by_role("dialog", name="Command reference")).to_be_visible()
    search = page.get_by_role("combobox", name="Search commands")
    search.fill("work only the second")
    page.keyboard.press("ArrowDown")
    page.keyboard.press("Enter")
    expect(page.locator(".lf-command-reference-meta")).to_have_text(
...
def test_banner_destinations_use_transient_target_overlays(browser, serve):
    ...
    key_faces = page.evaluate(
        """() => ['.lf-shortcut-more kbd', '.lf-shortcut-bar .lf-shortcut:not([hidden]) kbd']
    ...
    page.keyboard.press("?")
    page.keyboard.press("?")
    expect(page.locator(".lf-command-reference")).not_to_contain_text(
...
def test_a_key_the_runtime_binds_is_a_key_some_surface_names(browser, serve):
    ...
    page = open_page(browser, serve(NOTED_PAGE))
    line = page.locator(".lf-shortcut-bar")
    movement = line.locator('.lf-shortcut[data-lf-command-ids~="page.down"]')
    expect(movement).to_have_count(1)
    expect(movement).to_have_attribute(
        "data-lf-command-ids", re.compile(r"\bpage\.up\b")
    )
    ...
    expect(
        line.locator('.lf-shortcut:not([hidden])[data-lf-command-ids~="page.down"]')
    ).to_have_count(0)
    resized(page, 420, 800)
    ...
    expect(page.locator(".lf-command-reference")).to_contain_text("Caret browsing")
    page.keyboard.press("Escape")
...
def test_a_partially_shadowed_row_keeps_each_other_live_binding(browser, serve):
    ...
    expect(line.locator('[data-lf-command-ids~="page.down"]')).to_have_count(0)
...
def test_holding_a_key_repeats_only_where_the_press_is_a_walk(
    ...
    live_leaf("second", "A second leaf")
    page = open_page(browser, serve(ASKS_PAGE, comments=3))
...
def test_the_key_line_keeps_local_and_page_hints_and_progressively_reveals_the_rest(
    ...
    assert visible_hints.count() == 2, page.evaluate(
        """() => { const line = document.querySelector('.lf-shortcut-bar'); return {
          ...
          hints: [...line.querySelectorAll('.lf-shortcut')].map(el => ({
...
def test_the_resting_key_line_leads_from_the_page_to_target_selection(browser, serve):
    ...
    page = open_page(browser, serve(NOTED_PAGE))
    line = page.locator(".lf-shortcut-bar")
    shown = line.locator(".lf-shortcut:not([hidden])")
    ...
    for command in ("page.search.open", "page.down"):
        expect(
            line.locator(f'.lf-shortcut[data-lf-command-ids~="{command}"]')
        ).to_have_count(1)
        expect(
            line.locator(
                f'.lf-shortcut:not([hidden])[data-lf-command-ids~="{command}"]'
    ...
    expect(line.locator('[data-lf-command-ids~="reaction.open"]')).to_have_count(0)
...
def test_a_coarse_pointer_keeps_useful_status_without_keyboard_hints(browser, serve):
    """A phone drops key hints but keeps active navigation status."""
    context = browser.new_context(
...
def test_the_key_line_stands_in_a_band_of_its_own(browser, serve):
    ...
    page = open_page(browser, serve(FOOT_CONTROL_PAGE))
    ...
    assert page.evaluate(
        """() => {
          const more = document.querySelector('.lf-shortcut-more');
...
def test_the_walk_reaches_more_and_goes_on_after_the_line_has_repainted(browser, serve):
    ...
    for _ in range(24):
        page.keyboard.press("Tab")
        if page.evaluate(
            "() => document.activeElement?.classList?.contains('lf-shortcut-more')"
        ):
            break
...
def test_a_control_that_types_nothing_keeps_the_pages_keyboard(browser, serve):
    ...
    comment = line.locator(
        '.lf-shortcut:not([hidden])[data-lf-command-ids~="comment.create"]'
    )
    movement = line.locator(
        '.lf-shortcut:not([hidden])[data-lf-command-ids~="page.down"]'
    )
...
""",
    ...
    page = open_page(browser, serve(html, anchored=[("frames", "Choose a frame")]))
    ...
    assert "resolve" in thread_standing
    bounds = words.bounding_box()
    assert bounds is not None
    page.mouse.move(
...
def test_focus_paint_releases_every_text_box_crossed_before_a_frame(browser, serve):
    ...
    # Native disclosure makes each reply box reachable only while its own card is open,
    # so the summary clicks are the browser-owned transition between them.
    replies.evaluate_all(
        """boxes => {
          boxes[0].closest('.lf-thread').querySelector(':scope > .lf-thread-summary').click();
          boxes[0].focus();
          boxes[0].dispatchEvent(new Event('input', {bubbles: true}));
          boxes[1].closest('.lf-thread').querySelector(':scope > .lf-thread-summary').click();
          boxes[1].focus();
        }"""
    )
    shortcut_bar_text(page)
...
def test_a_key_on_screen_is_a_key_that_works(browser, serve):
    ...
    versions_route = help_el.locator(
        'tr[data-lf-command="version.open"] .lf-binding-sequence'
    )
    expect(versions_route.locator("kbd")).to_have_text(["g", "V"])
    expect(versions_route).to_have_attribute("aria-label", "g then Shift+v")
    sequence_control = help_el.locator('tr[data-lf-command="navigation.go-to.back"]')
    expect(sequence_control).to_have_class(re.compile(r"\blf-sequence-command\b"))
...
def test_r_resolves_the_focused_thread_while_x_is_unbound(browser, serve):
    ...
    The visible Resolve and Reopen controls retain their native keyboard routes."""
    url = serve(NOTED_PAGE)
    d = serve.page_dir
    ...
    expect(first.locator(":scope > .lf-thread-summary")).to_be_focused()
...
def test_r_resolves_a_thread_from_wherever_the_user_stands_in_it(browser, serve):
    ...
    expect(card(1).locator(":scope > .lf-thread-summary")).to_be_focused()
    ...
    card(2).locator(":scope > .lf-thread-summary").click()
...
def test_c_in_a_thread_reaches_that_threads_own_box(browser, serve):
    ...
    page.locator('[data-filter-value="resolved"]').click()
    page.locator(
        f'.lf-thread[data-id="{gone}"]:not([hidden]) > .lf-thread-summary'
    ).focus()
