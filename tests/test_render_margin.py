...
def test_a_thread_can_be_answered_in_the_margin_without_opening_comments(
    browser, serve
):
...
def test_only_a_page_with_threads_reserves_the_conversation_margin(browser, serve):
    ...
    page, errors = open_page(browser, serve(ASK_PAGE, events=[DECISION_ON_ASK]))
    ...
    events_model.append_event(serve.page_dir, COMMENT_ON_ASK)
    told(page)
