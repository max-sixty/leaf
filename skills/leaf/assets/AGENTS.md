...
| how much of a scroller the reader can see, and where a landing may put something | the scroller's shown band less the `.lf-pinned` covers stuck in it, or less its declared `scroll-padding` | `visibleBand` and `landingBand` in `geometry.js`; `shownRect`'s clip walk applies `visibleBand` at every ancestor, so whether something is on screen has one answer |
