export function createAskModel({
  ...
  // conversation takes no reply; only a later version that changes its target and leaves
  // the request answered unlocks the agent's resolve. A seat answer that held for good
  // would let a clarifying question retire a decision nobody made, invisibly to both
  // sides, which is what the log's own defaults exist to refuse.
  //
