...
- **#19** **Measure what Leaf saves an authoring agent** — Run a small blind authoring comparison against plain HTML, including the cost of the subsequent feedback cycle.
  ...
  **Follow-ups from the first run** (reproduced in the judge's defect lists and checked against the screenshots):
  ...
  - Log order: every Leaf author bounded the user's newest-first log with `data-bound="end"` and reversed it to match, one on a plain `<ol>`. The guide's Bounds paragraph offers that bound for a log and says it keeps the log on its newest line, which holds only with the newest line last, and the `lf-chronology` example runs oldest first. The plain authors, whose guide lacks the paragraph, kept the request's order. The result is defensible, but the judge cited it in 8 of 12 dashboard judgments. The guide should offer both orders, each with its bound, rather than implying one.
