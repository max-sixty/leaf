...
cd "$repo"
results="$repo/notes/mcp-apps/experiments/38/results"
run_dir="$(mktemp -d "$repo/.tmp/mcp-direct-38.XXXXXX")"
reference="/tmp/leaf-mcp-apps-reference-10195ad9"
mkdir -p "$results"
printf '%s\n' "$run_dir" > "$results/run-dir.txt"
npm install --prefix "$run_dir" --no-save --no-package-lock --silent esbuild-wasm@0.28.2 @modelcontextprotocol/ext-apps@1.7.5
