...
import { createTargetPaint } from "./target-paint.js";
import {
...
export function createAnchors(dependencies) {
    ...
    if (anchor.datum) {
      ...
      if (!datum.length) {
        ...
        if (!(virtual instanceof Element) || !containsAcross(source, virtual))
          return null;
        return resolvedElement({ element: virtual });
      }
      if (!anchor.quote) return resolvedElement({ element: datum[0] });
  ...
  function refreshAim() {
    ...
    const r = aimed && targetPaint.paintAim(aimed, target.surface);
    if (!r) {
      targetPaint.clearAim();
      paintInspect(null);
      return;
    }
    paintInspect(designIsOn() ? target : null, { left: r.left, top: r.top });
  }
  ...
  function paintAnchors(threads = buildThreads()) {
    ...
    for (const t of threads) {
      ...
      placed.set(t.root.id, found.place);
  ...
  const anchors = {
    ...
    refreshAim,
    traceTarget: targetPaint.traceTarget,
    dockSeats,
    paintAnchors,
    fragmentId,
    markAt,
