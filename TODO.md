...
- **Finish the Lit rendering cutover for Leaf-owned generated UI.** Move the remaining
  banner shelf/menu and thread-panel narrowing chrome one at a time onto Lit elements
  or templates fed only by read-only application projections. For each surface, preserve
