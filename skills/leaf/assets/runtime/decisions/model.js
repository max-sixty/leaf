...
export const decisionEntry = (el) => registry[el.tagName.toLowerCase()]?.["x-awaits"];
...
export function decisionSource(el) {
...
function decisions(kind) {
  ...
  const conversationDecisions = runtime.browser?.conversation?.decisions?.[kind] ?? [];
...
export const allDecisions = () => decisions("all");
export const openDecisions = () => decisions("reader");
export const unansweredDecisions = () => decisions("unanswered");
...
export function watchDecisions(owner, callback) {
