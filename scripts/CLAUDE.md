...
`highlight`, and `jsdiff`, so a clean `git status` after a run is the check that the
bundle still matches the script. `plot`, `pierre`, and `mcp-app` reach npm's resolver
for transitive dependencies and inherit its ranges, so a diff from one of those can be
an upstream patch rather than drift.
