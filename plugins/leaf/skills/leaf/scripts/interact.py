...
def _append_event(f, event: dict) -> dict:
    """Append through an already-held log lock, or return its accepted attempt."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def execute_event(self, event: dict) -> tuple:
        ...
        with flocked(self.page_dir / "comments.jsonl") as log:
            ...
            if "attempt" in event:
                event["author"] = "user"
                try:
                    existing = _matching_attempt(log, event)
