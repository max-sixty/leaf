...
def test_notification_playground_authored_preview_matches_default_controls_before_upgrade(
    ...
    try:
        ...
        expect(page.locator(".notification-healthy")).to_have_count(2)
        expect(page.locator(".notification-healthy").nth(0)).to_be_visible()
        expect(page.locator(".notification-healthy").nth(1)).to_be_visible()
        expect(page.locator(".notification-status-page")).to_have_count(2)
        expect(page.locator(".notification-status-page").nth(0)).to_be_visible()
        expect(page.locator(".notification-status-page").nth(1)).to_be_visible()
        expect(page.locator(".notification-watch").nth(0)).to_be_hidden()
        expect(page.locator(".notification-watch").nth(1)).to_be_hidden()
        expect(page.locator(".notification-slack").nth(0)).to_be_hidden()
        expect(page.locator(".notification-slack").nth(1)).to_be_hidden()
        expect(page.locator(".notification-owner").nth(0)).to_be_hidden()
        expect(page.locator(".notification-owner").nth(1)).to_be_hidden()
        for progress in (
            page.locator(".notification-progress").nth(0),
            page.locator(".notification-progress").nth(1),
        ):
            assert (
                progress.evaluate("node => getComputedStyle(node).backgroundColor")
                != "rgba(0, 0, 0, 0)"
            )
...
def test_notification_playground_uses_shared_bounded_regions_and_flows_when_narrow(
    ...
    panel_settled(page)
    expect(workspace).to_have_attribute("data-lf-reading-posture", "bounded")
    notification_box = page.locator("#notification-candidates").bounding_box()
...
def test_notification_configuration_becomes_a_commentable_local_artifact(
    ...
    expect(page.locator("#notification-strip-watch")).to_be_visible()
    expect(page.locator("#notification-healthy")).to_be_hidden()
    expect(page.locator("#notification-strip-healthy")).to_be_hidden()
    ...
    assert action["detail"]["instruction"] == (
        ...
        "Use the latency-watch state for slack; "
        "healthy means 100% rollout and latency-watch means held at 25%. Set owner "
        "visibility to true and title it Checkout rollout held at 25%. Show me the "
...
.notification { border-top: 5px solid #b6533c; padding: 12px; }
</style>
...
def test_notification_playground_export_flows_at_another_width_and_on_paper(
    ...
    expect(playground.locator(".notification-card")).to_have_count(2)
    expect(playground.locator("#notification-card")).to_have_accessible_name(
        "Escalation sent"
    )
    expect(playground.locator("#notification-strip")).to_have_accessible_name(
        "Escalation sent"
    )
    expect(playground.locator("#notification-card")).to_be_visible()
    expect(playground.locator("#notification-strip")).to_be_visible()
