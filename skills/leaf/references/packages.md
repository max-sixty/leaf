...
A widget whose reader changes something records each change as an `action` event in the
...
widget". A verb that lets the reader add real children declares
...
with `leaf report`, and it stands until a version answers it. A reader's action at the
