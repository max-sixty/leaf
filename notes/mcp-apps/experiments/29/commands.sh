...
results="$repo/notes/mcp-apps/experiments/29/results"
run_dir="$(mktemp -d "$repo/.tmp/mcp-app-experiment-29.XXXXXX")"
...
./bin/leaf version stamp --text "Baseline decision" "$page_dir"
perl -0pi -e 's/The monolith split leaves/The monolith extraction leaves/' \
  "$page_dir/index.html"
./bin/leaf version stamp --text "Name the extraction" "$page_dir"
...
uv run python notes/mcp-apps/experiments/29/observe.py "$page_dir" \
