"""The Codex App Server carrier: its connection, its readings, and its deliveries.

Two hosts drive one protocol. The detached adapter in `codex.py` observes a Codex
task it does not own; the website's embedded host in `worker/server.py` owns the
tasks it starts. Everything both need is here: the local transport and the request
shapes one Leaf turn is opened with, the fold from provider notifications into
activity and final-answer readings, the writers that put those readings on a claimed
page, and the durable records a delivery passes through.

A delivery record under the state home is the handoff between Leaf capturing a
reader's moves and a Codex carrier taking them. One record is offered once, accepted
once, and receipted per page batch, whichever transport carried it — an App Server
turn or the `codex queue` command — so its lifecycle lives here rather than beside
either carrier. The immutable payload itself belongs to `delivery`; what this module
keeps is which task holds it and how far it has got.

Which delivery is offered, and when, is the carrier's own policy: the adapter's queue
