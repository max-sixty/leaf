...
A suggestion is for wording the reader could reasonably prefer as it stands. A
...
rewrite would only restore a page that is wrong or out of date, so the reader has
...
the reader, and point at its material additions on the page with a temporary
...
An Ask the reader answered and you have acted on is finished work too. Move it
into `Complete` with the reader's answer standing, and put anything still worth
asking in a new Ask under new ids. An `lf-options` group takes `settled` there,
which collapses it to the pick. The answered Ask is the record of what the reader
...
no longer revisits it. Keep a section live while the reader is still commenting
there.
