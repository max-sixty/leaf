...
def test_a_shared_passage_steps_between_single_conversation_cards(browser, serve):
    ...
    assert (
        controls[".lf-margin-preview-nav"]["middle"]
        < controls[".lf-resolve"]["middle"]
    ), controls
    assert (
...
def test_the_shipped_long_thread_keeps_the_margin_and_its_height(browser, serve):
    ...
    expect(preview).to_be_visible()
    expect(preview).to_have_attribute("aria-label", "Conversation for iOS reconnect stall")
    ...
    assert geometry["metadataLeft"] == pytest.approx(
        geometry["cardLeft"] + 17, abs=0.5
    )
