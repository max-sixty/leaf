...
def read_text_arg(page_dir: Path, text) -> str:
    ...
    cannot claim a widget — but it can name a file, and a picture the directory
    hasn't got is a broken image in an append-only log for as long as the page
    exists. `check_markup` asks that of `--markup` and runs only when one is given;
    this asks it of the words beside it."""
...
def check_markup(page_dir: Path, kind: str, markup: str, events: list) -> _StructParser:
    ...
    escaped, so it cannot claim a widget — but it can still name a file, which
    `read_text_arg` asks about wherever a body arrives. Exits with what's wrong."""
