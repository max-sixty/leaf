...
| Work lands on a handed-over page | A plan, a list of problems found, or a refuted finding, with a reader's pick standing, and a task line that only asks to note what merged | The title and headings state the present; finished items sit in a collapsed section rather than marked done in the open list; the answered Ask moves there under the words it was picked under and what remains is a new Ask; a milestone rail keeps its done members; a reader-owned draft still gets an `lf-suggestion`. |
