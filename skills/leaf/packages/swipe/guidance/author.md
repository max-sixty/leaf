...
Use a swipe deck when the reader can classify several independent technical proposals quickly; use ordinary options when the proposals need side-by-side comparison. One deck is one Ask: wrap it in `lf-ask` and lead with the heading that states the question. Pressing `a` then lands on that question and exposes the deck's ← Pass and → Keep actions inline. The final `finish` classification closes the Ask through the deck's declared empty-queue condition; the server tests it after the recorded move, so a stale rapid gesture cannot close a queue whose earlier card returned.
