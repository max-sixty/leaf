...
| a host wait's start that no tool hook has named | a lock on `sessions/<id>.started` | the `leaf wait` process, taken with the session's wait lease under `sessions/<id>.started.lock` and held for its life | the `PostToolUse` hook removes the file under that same lock when it names the start; process exit, after which `sweep` removes the file under that lock |
