...
exported document resolves them to bytes captured for that revision. The server never
serves mutable author source directly. Relative module imports, CSS imports, and CSS URL
dependencies stay within the captured graph; a dependency that escapes the page source
root or cannot be resolved refuses activation. The revision store may deduplicate bytes
by digest, but deduplication is not part of the author contract.

Every live revision activates as a complete document. Markup-only DOM replacement and
executable reload are not separate paths. Leaf may optimize activation later only if a
measurement justifies it and the optimized path preserves the same lifecycle.
...
to the revision that uses it. A page-owned upgraded element takes its entry module from
`<page>/page/widgets/<tag>.js`. The contribution is not an implicit package: it has no
install name, no package selection, and no reach beyond that page instance.
...
1. Complete #1's revision-correctness cutover using one document activation path.
2. Capture page-owned modules, styles, and local dependencies with each revision.
3. Compose revision-bound page declarations through the existing registry and event
   machinery (#2).
4. Separate design intent from source ownership and expose identity controllers needed
   by #7.
5. Build interactive export on the captured revision graph (#5).

Each slice replaces the corresponding future-tense rules here with implementation
contracts beside the owning code. The final slice deletes this note and removes its
entry from `TODO.md`.
