...
results="$repo/notes/mcp-apps/experiments/13/results"
run_dir="$(mktemp -d "$repo/.tmp/mcp-app-experiment-13.XXXXXX")"
...
shasum -a 256 skills/leaf/assets/vendor/mcp-compact-app.html >"$results/bundle.sha256"
...
uv run python notes/mcp-apps/experiments/13/observe.py "$run_dir/page" \
