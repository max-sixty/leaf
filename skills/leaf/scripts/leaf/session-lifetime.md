...
| wait lease, holding the holder's start token | `waiter.lock`, or `sessions/<id>.wait` for a host session | the live `leaf wait` process, held open for its life; each holder writes a fresh token on taking it | process exit; `sweep` removes the unheld state-home file |
| the wait start a tool hook last named | `sessions/<id>.told` | the `PostToolUse` hook, when it names a token the lease holds | a later start token in the lease; `sweep` removes it once the lease is unheld |
...
| Codex delivery record | the host state home's session records | the detached adapter or an embedded App Server host | an unaccepted record is inactive while the session owns no page; an accepted record moves under `history/` after every batch is receipted; `sweep` removes either once every page it names is gone |
| Leaf delivery | `<state-home>/deliveries/<id>.json` | any carrier freezes the host-neutral envelope before presenting it | never while a page it names stands; every transport resolves the same immutable id. `sweep` removes it once they are all gone |
