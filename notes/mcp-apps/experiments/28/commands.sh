...
results="$repo/notes/mcp-apps/experiments/28/results"
run_dir="$(mktemp -d "$repo/.tmp/mcp-app-experiment-28.XXXXXX")"
...
uv run pytest -n0 tests/test_render_controls.py \
  -k test_a_page_that_asks_nothing_carries_no_terminal_control
...
uv run python notes/mcp-apps/experiments/28/observe.py "$run_dir/page" \
