...
animations to empty. `reservations_taken` waits for the room the panel's settlement
controls hold for the widest word they can say; that room is measured off the box the
open panel gives them, so it lands a ResizeObserver delivery and a frame after
everything `panel_settled` states, and a geometry read taken on either side of it
compares two different buttons. `resized` waits for the resize event to reach
listeners and then for one rendering update behind it; a new viewport size says only
that the browser resized, not that page layout handled it, and the event says only
that the page was told — the document's own scrolling area is published in the update
after the one the event arrived in.
