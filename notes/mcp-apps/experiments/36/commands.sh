...
cd "$repo"
results="$repo/notes/mcp-apps/experiments/36/results"
run_dir="$(mktemp -d "$repo/.tmp/mcp-direct-36.XXXXXX")"
reference="/tmp/leaf-mcp-apps-reference-10195ad9"
mkdir -p "$results"
printf '%s\n' "$run_dir" > "$results/run-dir.txt"
"$repo/bin/leaf" --version
"$repo/bin/leaf" page init "$run_dir/page"
cp examples/design-decision.html "$run_dir/page/index.html"
"$repo/bin/leaf" version stamp "$run_dir/page" --text "Direct MCP resource probe"
node scripts/mcp-app/direct-build.mjs "$run_dir/page" "$reference/node_modules" "$run_dir/bundle.js"
...
uv run python notes/mcp-apps/experiments/36/observe.py "$run_dir/page" > "$results/reference-host.json"
