...
one set of tracks: typically a body beside a side track as one `lf-grid` of
...
- A sheet, and so a workspace, keeps no rail. Its markers stand as pins over the page at
  ...
  where 32px of padding is free there, and otherwise covers what is missing.
...
`data-rail="right"` on `main` keeps the rail on a sheet, and `data-rail="none"` gives a
document's right margin to something else: a page that hangs `aside.sidenote`s on the
right writes it, since the notes and the rail claim the same strip.
