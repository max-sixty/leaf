...
results="$repo/notes/mcp-apps/experiments/2/results"
run_dir="$(mktemp -d "$repo/.tmp/mcp-app-experiment-2.XXXXXX")"
...
uv run python notes/mcp-apps/experiments/2/observe.py "$run_dir/page" \
  >"$results/reference-host.json"

# Open the generated compact-app screenshot:
# open notes/mcp-apps/experiments/2/results/compact-ask-420x360.png
