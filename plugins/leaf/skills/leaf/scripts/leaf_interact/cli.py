...
def create_cli(api):
    ...
    def init(dir: str, selected: tuple[str, ...], no_packages: bool) -> None:
        ...
        if selected and no_packages:
            raise click.UsageError(
                "--package and --no-packages cannot be used together"
            )
    ...
    @click.option(
        "--render", is_flag=True, help="also check the rendered page in Chrome"
    )
    ...
    def status(dir: str, state: str, detail: str, on: str | None) -> None:
        ...
        with PageTransaction(page_dir) as page:
            ...
            if pending:
                ...
                sys.exit(
                    prefix
                    + "`leaf wait` prints them and returns at once when events are "
    ...
    @cli.command(
        short_help="Open an agent thread — on a passage, or on the page whole."
    )
    @click.argument("dir", metavar="PAGE")
    @click.option("--quote", help="passage text from the published version")
    @click.option(
        "--section", metavar="ID", help="element ID to anchor or scope --quote"
    )
    @click.option("--text", help="comment text (default: stdin)")
    @click.option(
        "--markup", help="widget markup to render after the text, validated here"
    )
    ...
    @click.argument("dir", metavar="PAGE")
    @click.option(
        "--to", required=True, metavar="ID", help="comment or reply ID to answer"
    )
    ...
    def reply(dir: str, to: str, text: str, markup: str) -> None:
        ...
        print(
            json.dumps(
                cmd_reply(resolve_dir(dir), to, text, markup), ensure_ascii=False
            )
        )
