/* Projection reconciliation and undo.

   Rendering version V means making its DOM equal the log's desired projection. Each
   `(owner widget, unit, facet)` keeps its last surviving action or report, with a reader
   action outranking provisional agent news on the same coordinate. The framework
   composes those winners with authored state, including complete ordered containers,
   before widgets render each final facet map through renderState(state).
   ...
   omits can un-make a decision by itself; the repository's CLAUDE.md carries why.
   Acknowledgement is not assent: only a version can say what the agent did with an
   action, and saying it is `version check`'s business (restatement_errors), never
   inferred here from silence. */
import {
  authoredFacet,
  authoredStates,
  stateCoordinate,
...
import { pageShifted } from "./anchors.js";
...
export function resetAuthoredPage() {
...
export function coordinateProjectionCommitted(projection, entry) {
...
export function projectionCommitted(projection, e) {
...
export function releaseProjectedOutbox() {
...
export const projectionDeferred = () => deferredProjection;
...
export function reconcileKnownState() {
