...
that day and will drift. The landscape was first swept between 2026-07-31 and
2026-08-29, and re-swept on 2026-09-22. The next section records what the second
sweep found; the sections after it stand as read on their own dates, and where the two
...
Read on 2026-09-22 from repositories (`gh api`), changelogs, release notes and docs.
Star counts are from that day.

The largest change is first-party. [Claude Code Artifacts](https://code.claude.com/docs/en/artifacts)
grew a server-side runtime: the [Claude Code changelog](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md)
for v2.1.265–v2.1.280 (2026-09-08 to 09-22) adds a shared per-page database with
access rules, a viewer-profile scope, stale-publish refusal with a merge hand-back,
watching up to ten artifacts, and `/design` canvases. The artifact runtime seen from a
Claude Code session on 2026-09-22 also offers pages that publish new versions of
themselves, `sample` (the page asks Claude), uploads, live presence, and four artifact
types (Slides, Docs, Design, Design System). The public docs still say "no backend" and
do not mention the database, so general availability is unconfirmed. Comments still
need Team or Enterprise and are off on public links, and their anchor is a CSS path plus
x/y or a name the page registers, not a text quote. An open bug,
[#92618](https://github.com/anthropics/claude-code/issues/92618), reports comments sent
...
- **MCP Apps reached Codex.** Codex Desktop renders MCP Apps, and Codex's app-server
  keeps their UI metadata in tool-call history
  ([openai/codex#45805](https://github.com/openai/codex/pull/45805)). The Claude Code CLI
  still does not ([#95149](https://github.com/anthropics/claude-code/issues/95149)).
  [ext-apps v2.0.0](https://github.com/modelcontextprotocol/ext-apps/releases) (09-08)
  moved to the split MCP SDK packages with the wire protocol unchanged; the stable spec
  is still 2026-01-26, and the draft's app-provided tools remain unmerged. The
  [client matrix](https://modelcontextprotocol.io/extensions/client-matrix) is the
  current host list.
- **AG-UI froze at 1.0** on 2026-09-17
  ([ag-ui#2783](https://github.com/ag-ui-protocol/ag-ui/pull/2783)): schema-first, 31
  event types with subagent events added, generated SDKs and a conformance corpus.
- **The local review tools converged on leaf's loop.**
  [lavish-axi](https://github.com/kunchenguid/lavish-axi) added durable conversation
  history, a revision legend and restart-surviving reviews;
  [Plannotator](https://github.com/backnotprop/plannotator) an append-only feedback
  archive and PR comments that survive pushes, marked Outdated rather than moved;
  [crit](https://github.com/tomasz-tomczyk/crit) line-anchored comments across rounds
  and `crit listen`. [reviewable-html-workbench](https://github.com/u-ichi/reviewable-html-workbench)
  has a schema-validated document model and a gate that blocks updates while threads
  are open — the nearest thing to a checked vocabulary outside leaf, with no widgets.
- **The declarative formats grew without consolidating.**
  [json-render](https://github.com/vercel-labs/json-render) shipped v0.21.0 (09-18,
  TanStack Start renderer, experimental [Jev](https://json-render.dev/docs/jev) choice
  model); Thesys released [OUI-1](https://github.com/thesysdev/openui/blob/main/docs/content/blog/oui-1.mdx),
  an open-weight 26B model fine-tuned to write OpenUI Lang, with a public
  [benchmark](https://github.com/thesysdev/generative-ui-bench), and folded C1 and Crayon
  into [OpenUI](https://github.com/thesysdev/openui); [A2UI](https://github.com/a2ui-project/a2ui)
  1.0 is still a candidate, now [targeted at Q4 2026](https://a2ui.org/roadmap/), and gained
  two model-facing inference formats, Express (a text notation) and Elemental (HTML tags),
  that compile to its JSON. Open-JSON-UI is gone: CopilotKit retired the page on
  2026-09-17 and redirects it to A2UI.
- **Two directions show across the formats.** The notation moves toward what models
  write well — OpenUI Lang, Express, Elemental — which is the bet on HTML fluency this
  note's introduction makes. And a counter-current has the model choose among authored
  components rather than generate a tree: Jev, [proteus](https://github.com/sanskritifarswal/proteus),
  [decided-ui](https://github.com/pjdurden/decided-ui). Neither direction touches the
  log: none has versions, replay or anchored remarks.

| Project | Stars 08-2x → 09-22 | Latest |
| ------- | ------------------- | ------ |
| [CopilotKit](https://github.com/CopilotKit/CopilotKit) | 36.9k → 37.5k | [v1.73.3](https://github.com/CopilotKit/CopilotKit/releases), 09-22 |
| [AG-UI](https://github.com/ag-ui-protocol/ag-ui) | → 16.0k | spec 1.0, 09-17 |
| [A2UI](https://github.com/a2ui-project/a2ui) | 16.2k → 16.5k | v0.9 tag; v1.0 candidate |
| [json-render](https://github.com/vercel-labs/json-render) | 16k → 18.1k | v0.21.0, 09-18 |
| [OpenUI](https://github.com/thesysdev/openui) | 8.4k → 9.8k | OUI-1, 09-08 |
| [Hashbrown](https://github.com/liveloveapp/hashbrown) | 719 → 723 | 0.5.0, 07-13; core moving onto AG-UI |
| [Plannotator](https://github.com/backnotprop/plannotator) | 7.9k → 8.9k | v0.27.18, 09-22 |
| [effective-html](https://github.com/plannotator/effective-html) | 1.7k → 3.3k | — |
| [lavish-axi](https://github.com/kunchenguid/lavish-axi) | → 3.8k | v0.1.78, 09-22 |
| [herdr](https://github.com/herdrdev/herdr) | 31.7k → 40.3k | v0.9.1, 09-16 |
| [crit](https://github.com/tomasz-tomczyk/crit) | → 1.1k | v0.20.2, 09-18 |
| [Agentation](https://github.com/benjitaylor/agentation) | → 4.7k | v3.1.0, 09-21 |
| [assistant-ui](https://github.com/assistant-ui/assistant-ui) | → 12.3k | `react-generative-ui` since 06-01 |
| [Tambo](https://github.com/tambo-ai/tambo) | → 11.2k | main quiet since 08-05 |
| [Vercel AI SDK](https://github.com/vercel/ai) | → 26.9k | ai@7.0.111; no generative-UI work |

New since August; apart from OpenBot, none has passed about 150 stars:
...
- [assistant-ui's generative UI](https://github.com/assistant-ui/assistant-ui) — missed
  ...
  tool.
- [OpenBot](https://github.com/CopilotKit/OpenBot) (5.4k stars, spun out 08-17) — CopilotKit's "AI coworkers" over
  AG-UI; not a UI format.
- [accountable-review](https://github.com/wyeworks/accountable-review) — a Claude Code
  plugin publishing a PR as an HTML review map.
- [MarkuprPlus](https://github.com/hashfunction/MarkuprPlus) — a narrated screen
  recording turned into structured markdown for the agent.
- [pi-plans](https://github.com/MaxInGaussian/pi-plans) — versioned markdown plans with
  a human in the loop, for Pi.
- [charrette](https://github.com/Ovich/charrette) — "living local documents".
- Self-hosted artifact publishers: [sidequery/artifacts](https://github.com/sidequery/artifacts),
  [portego](https://github.com/laguna-engineering/portego).
- Annotation clones in Plannotator's or Agentation's shape:
  [comments-harness](https://github.com/denysosadchyi/comments-harness),
  [Remarc](https://github.com/metedata/Remarc), [anno](https://github.com/philmingdao/anno),
  [mdtation](https://github.com/janniks/mdtation).

Unverified in this sweep: whether the Artifact database, `sample` and self-publishing
are generally available; when Codex Desktop began rendering MCP Apps; an
`androidx.a2ui` module seen only through a
[DroidKaigi talk repository](https://github.com/JAICHANGPARK/droidkaigi2026-a2ui).
...
reach, roughly in order of how badly the omission dates this one. The 2026-09-22 sweep
above checked each for movement; none has become a page-and-log loop:

- **[crit](https://github.com/tomasz-tomczyk/crit)** — a local single Go binary, bound to loopback, no config or login; the agent
  launches it and blocks on the review rather than serving a page and watching it.
- **[reviewable-html-workbench](https://github.com/u-ichi/reviewable-html-workbench)** — a Claude Code and Codex plugin running nearly this loop,
  down to the agent replying in-thread in the browser.
- **[MCP Apps](https://modelcontextprotocol.io/extensions/client-matrix)** — the official MCP UI extension since January 2026, built on the earlier
...
- **Agent control layers** — [Paseo](https://github.com/getpaseo/paseo), [Nimbalyst](https://github.com/nimbalyst/nimbalyst), Agentastic, [OpenWork](https://github.com/different-ai/openwork) and the like: GUIs
...
- **Human-in-the-loop inboxes** — LangChain's [Agent Inbox](https://github.com/langchain-ai/agent-inbox) and `humanInTheLoopMiddleware`,
  [HumanLayer](https://github.com/humanlayer/humanlayer) routing approvals to Slack or email. Approve, edit, reject or respond, on a
