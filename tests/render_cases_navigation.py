...
<lf-ask id="rows-decision"><h2>When should it be costed?</h2>
...
""",
    ...
    # Enough page below the decision that it can be scrolled up under the banner, which is where
...
<lf-ask id="picks-decision"><h2>Should we keep the mounts?</h2>
...
NESTED_SUGGESTION = (
    SUGGESTION_PAGE.replace(
        ...
        '<lf-ask id="blend-decision"><h2>Which seed blend?</h2>'
...
def live_watcher(page_dir, page):
    ...
    lease = leases_model.take_waiter_lease(
        leases_model.waiter_lease_path(page_dir, session["id"] if session else None)
    )
    assert lease
    told(page)
    try:
